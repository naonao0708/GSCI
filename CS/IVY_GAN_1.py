...
if __name__ == "__main__":
    ...
    # 数据加载
    x = np.load("./Result/GSE103744/GSE103744_VAE-GAN.npy")
    x=x.T

    x = x.astype(np.float64)
    # z = np.load("./Data/GSE147326/GSE147326_log.npy")
    z = np.load("./Data/GSE103744_log.npy")
    ...
    while (itr < 1):
        ...
        # 初始化
        k = min(int(x.shape[1] * 3), 170)  # 稀疏编码的基函数数量
        ...
        np.savetxt('./Result/GSE103744/GSE103744_VAE+GAN+IVY.npy', xa, delimiter=',')

        df = pd.DataFrame(xa)
        df.to_csv("./Result/GSE103744/GSE103744_VAE+GAN+IVY.csv")
        np.save('./Result/GSE103744/GSE103744_VAE+GAN+IVY.npy', xa)
        ...
        global_metrics = ['iteration', 'mse', 'mae', 'pcc', 'cs']
        df_global = df[[col for col in df.columns if col in global_metrics]]
        df_global.to_csv("./Result/GSE103744/GSE103744_VAE+GAN+IVY_global.csv", index=False)
