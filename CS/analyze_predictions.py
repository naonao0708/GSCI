...
def compare_distances(A, B, random_samples=[], s=200, pvalues=False):
    if len(random_samples) == 0:
        ...
        random_samples[:min(s, A.shape[1])] = True # s默认值为200 ，如果A的列数小于200，则选择全部列
        np.random.shuffle(random_samples)
    dist_x = distance.pdist(A[:, random_samples].T, 'euclidean') #欧几里得距离
    dist_y = distance.pdist(B[:, random_samples].T, 'euclidean')
    pear = pearsonr(dist_x, dist_y)  # 计算皮尔逊相关系数
    spear = spearmanr(dist_x, dist_y) # 计算斯皮尔曼相关系数
    if pvalues:
        return pear, spear
    else:
...
def correlations(A, B, pc_n=100):
    p = (1 - distance.correlation(A.flatten(), B.flatten())) #Pearson相关系数
    spear = spearmanr(A.flatten(), B.flatten())  # Spearman相关系数
    dist_genes = np.zeros(A.shape[0])
    for i in range(A.shape[0]):
        dist_genes[i] = 1 - distance.correlation(A[i], B[i])
    pg = (np.average(dist_genes[np.isfinite(dist_genes)])) # 基因间距离
    dist_sample = np.zeros(A.shape[1])
    for i in range(A.shape[1]):
        dist_sample[i] = 1 - distance.correlation(A[:, i], B[:, i])
    ps = (np.average(dist_sample[np.isfinite(dist_sample)])) # 样本间距离
    pc_dist = []
    if pc_n > 0:
        ...
        pc_dist = np.array(pc_dist)  # 主成分距离
