...
def merge_vaegan_ivycs(x1, x3, original_x, threshold=1e-5):
    """
    融合 VAE-GAN 输出与 IVYCS 输出：
    - 在 original_x 的 Dropout 区域（值 < threshold）用 x3 替换 x1；
    - 其他区域保留 x1。

    参数：
        x1: VAE-GAN 输出 (已归一或未归一均可)
        x3: IVYCS 输出（基于 x1 输入）
        original_x: 原始输入（含 Dropout 的）
        threshold: 判定 Dropout 的阈值，通常为 0 或 1e-5

    返回：
        融合后的矩阵，适用于最终评估
    """
    assert x1.shape == x3.shape == original_x.shape, "Shape mismatch!"

    # 构建 dropout 区域 mask（Dropout为True）
    dropout_mask = original_x < threshold

    # 复制 x1 为最终输出
    merged_output = x1.copy()

    # 替换 dropout 区域为 x3（IVYCS 输出）
    merged_output[dropout_mask] = x3[dropout_mask]
...
if __name__ == "__main__":
    ...
    x = np.load("./Data/GSE123358/results/GSE123358_VAE-GAN.npy")
    ...
    while (itr < 1):
        ...
        while gen <= maxItr:
            ...
            xa, phi, y, w, d, psi = recover_system_knownBasis(x, measurements, sparsity, Psi=UF, snr=SNR,
                                                              use_ridge=False)
            # 使用 VAE-GAN 输出 x 作为 x1，z 是原始输入，xa 是 IVYCS 输出 → 进行融合
            xa = merge_vaegan_ivycs(x1=x, x3=xa, original_x=z)
            # 执行完整评估
        ...
        # np.savetxt('./Result/GSE123358/GSE123358_VAE+GAN+IVY.csv', xa, delimiter=',')
        # np.save('./Result/GSE123358/results/GSE123358_VAE+GAN+IVY.npy', xa, delimiter=',')
        # np.savetxt('./Result/GSE124989/GSE124989_VAE+GAN+IVY.csv', xa, delimiter=',')
        ...
        # df_global.to_csv("./Result/GSE147326/GSE147326_VAE+GAN+IVY_global.csv", index=False)
        # df_global.to_csv("./Result/GSE124989/GSE124989_VAE+GAN+IVY_global.csv", index=False)
        # df_global.to_csv("./Result/GSE123358/GSE123358_VAE+GAN+IVY_global.csv", index=False)
        ...
        # df_gene.to_csv("./Result/GSE147326/GSE147326_VAE+GAN+IVY_gene.csv", index=False)
        # df_gene.to_csv("./Result/GSE124989/GSE124989_VAE+GAN+IVY_gene.csv", index=False)
        # df_gene.to_csv("./Result/GSE123358/GSE123358_VAE+GAN+IVY_gene.csv", index=False)
